...
- `fix-kludged-time.rb` — **critical timezone correction.** The SUMO/Kitsune API returns Pacific-time timestamps mislabeled with a `Z` (UTC) suffix (Kitsune bugs [#3961](https://github.com/mozilla/kitsune/issues/3961), [#3946](https://github.com/mozilla/kitsune/issues/3946)). `kludge_time_from_bogusZ_to_utc` reinterprets them as `America/Los_Angeles` (handling PST/PDT) and converts to a correct offset. Question/answer `created`/`updated` fields are always passed through this. Date-range queries also pad ±1 day (Kitsune [#3686](https://github.com/mozilla/kitsune/issues/3686)) and re-filter in Ruby.
